...
can run on it.  File::Util's ability to support multiple platforms will
probably be affected as it begins to include more and more support for
Unicode, however.
...
recursive directory listings against regular expressions,
I<it is not a file-finder/searcher.>

If you are looking for a tool that allows you to search rapidly through
directory trees, B<I<NOT just filtering results>>, take a look at L<File::Find>.
There are many up-and-comers in the Perl file searching utilities niche that you
can also try, however they are still hard pressed to meet with the raw
performance and speed of File::Find in very large file trees.
...
It can also return a heirarchal datastructure of the file tree you ask it to
walk.
...
Return results with the preceeding file paths intact, relative
...
the C<max_dives> limit is reached. I<See L<max_dives()|/max_dives>>
...
B<Note:> Restrictions imposed by the current "read limit"
...
derived from the conents of C<$^O>, or if C<$^O> can not be found, from the
