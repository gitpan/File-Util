...
=head2 Why not first check if the file is writeable/can be created
   ...
   if ( $f->can_write( '/root/some/file.txt' ) ) {
...
that the C<$MAX_DIVES> limit applies here too (see L<File::Util> documentation),
which you can override manually by setting the C<max_dives> option:

   # set max recursion limit to an integer value as shown below
   $f->list_dir( '/tmp' => { as_tree => 1, recurse => 1, max_dives => 1234 } );
